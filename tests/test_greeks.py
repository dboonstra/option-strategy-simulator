...
class TestBlackScholes(unittest.TestCase):
    ...
    def test_valid_call(self):
        price, delta, theta, vega, gamma = black_scholes(100, 100, 30, "C", 0.2)
        self.assertIsNotNone(price); self.assertIsNotNone(delta); self.assertIsNotNone(theta)
        assert 0 <= delta <= 1
    
    def test_valid_put(self):
        price, delta, theta, vega, gamma = black_scholes(100, 100, 30, "P", 0.2)
        self.assertIsNotNone(price); self.assertIsNotNone(delta); self.assertIsNotNone(theta)
        assert -1 <= delta <= 0
    
    def test_zero_time_to_expiry(self):
        price, delta, theta, vega, gamma = black_scholes(100, 100, 0, "C", 0.2)
        self.assertEqual(price, 0); self.assertEqual(delta, 0); self.assertIsNone(theta)

        price, delta, theta, vega, gamma = black_scholes(100, 110, 0, "P", 0.2)
...
class TestImpliedVolatility(unittest.TestCase):
    ...
    def test_valid_iv(self):
        iv, delta, theta, vega, gamma = implied_volatility_newton_raphson(100, 100, 30, 5, "C")
